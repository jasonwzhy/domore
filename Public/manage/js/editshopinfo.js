...
function addpic(imgpath,albumsid){
	$("#addimg").before('<div class="col-xs-4 shoppic"><div class="picitem" id="'+albumsid+'" data-toggle="modal" data-target=".bs-example-modal-lg" onclick="onshoppic(this);" style="background-image:url(http://'+imgpath+')"></div></div>');
};
...
$("#subbtn").click(function(){
	...
	var shopid = $("#subbtn").attr("sid");
	$.post("/manage/agentshop/subshopinfoedit",
		{
			agetnshopid:shopid,
			shopdesc:shopdesc,
		...
		function(ret){
			window.location.href="/manage/agentshop/myshop/";
			// if (ret.error != "") {//success
			// 	alert(ret.error);
